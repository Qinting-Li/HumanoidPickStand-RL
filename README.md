...
* regression tests for broken assets, API errors, contact regressions, and NaN/Inf simulation failures
...
The MuJoCo scene disables robot self-collision for imported CAD mesh geoms while preserving robot-ground, robot-object, and object-ground contacts. It also adds simple box collision geoms for both soles and palms. This removes internal contacts at adjacent links and gives the benchmark stable support contacts.
...
Robot geoms collide with world/object but not robot. Object geoms collide with robot/world. World geoms collide with robot/object.
...
The PD controller is a diagnostic baseline, not a full pick-and-stand controller. It checks whether a conservative neutral pose can run without numerical instability and reports whether the humanoid remains upright.
...
The test suite checks asset references, MuJoCo loading, Gymnasium reset/step behavior, contact masks, object-ground contact, PD action shape, and 10-second PD standing without NaN/Inf states.

## Known Limitations

* The current PD controller is a diagnostic standing baseline, not a full pick-and-stand controller.
* Random policies still fall; this is expected and is not considered a simulator bug.
